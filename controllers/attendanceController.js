  ...
  try {
    ...
    if (minutesBeforeEnd) {
      ...
      if (now < gateOpenTime) {
        ...
        return res.status(403).json({
          success: false,
          message: `Attendance locked. Opens in ${minutesLeft} minutes.`,
        });
      }
    }

    // ===============================
    // 6. SPEED (ANTI-TELEPORT)
    // ===============================
    ...
    if (lastLocation.rows.length > 0) {
      const prev = lastLocation.rows[0];
    ...
    if (fraudScore > 70) {
      await pool.query(
        `INSERT INTO attendance_logs 
         (user_id, matric_number, course_id, schedule_id, device_uuid, status, distance_m, accuracy_m, device_info)
         VALUES ($1,$2,$3,$4,$5,'SUSPECTED_SPOOF',$6,$7,$8)`,
        [
          userId,
          matric_number,
          course_code,
          ...
          0,
          accuracy,
          JSON.stringify({ device_name, os_version, app_version }),
        ],
      );
    ...
    // 9. GEOFENCE CHECK
    // ===============================
    const allowedRadius = schedule.radius_m || GEOFENCE_MAX_M;
    ...
    if (distance > allowedRadius) {
      await pool.query(
        `INSERT INTO attendance_logs 
         (user_id, matric_number, course_id, schedule_id, device_uuid, status, distance_m, accuracy_m, device_info)
         VALUES ($1,$2,$3,$4,$5,'OUTSIDE_RANGE',$6,$7,$8)`,
        [
          userId,
          matric_number,
          course_code,
      ...
      return res.status(403).json({
        success: false,
        message: `Too far (${distance.toFixed(0)}m).`,
      });
    }
    ...
    const dup = await pool.query(
      `SELECT * FROM attendance_logs 
       WHERE user_id = $1 AND schedule_id = $2 AND status = 'VALID'`,
      [userId, schedule_id],
    );
...
exports.getAttendanceByScheduleId = async (req, res) => {
  ...
  try {
    ...
    const attendance = await classScheduleModel.getScheduleAttendanceDetails(
      id,
    ...
    return res
      .status(200)
      .json({ success: true, attendance: mappedAttendance });
  } catch (err) {
    console.error(err);
    return res
      .status(500)
