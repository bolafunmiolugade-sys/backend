...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (navCoursesBtn)
    navCoursesBtn.addEventListener("click", () => switchTab("courses"));
  if (navLecturersBtn)
    navLecturersBtn.addEventListener("click", () => switchTab("lecturers"));
  if (navSchedulesBtn)
    navSchedulesBtn.addEventListener("click", () => switchTab("schedules"));
  if (navStudentsBtn)
    navStudentsBtn.addEventListener("click", () => switchTab("students"));
  if (navAttendanceBtn)
    navAttendanceBtn.addEventListener("click", () => switchTab("attendance"));
  ...
  try {
    ...
    if (data.success) {
      ...
      document.querySelectorAll(".edit-course-btn").forEach((btn) => {
        btn.addEventListener("click", () =>
          openEditCourseModal(btn.getAttribute("data-id")),
        );
      });
...
async function openEditCourseModal(id) {
  try {
    ...
    if (data.success) {
      ...
      document.getElementById("edit_course_dept_code").value =
    ...
    const courseNames =
      l.courses.length > 0
        ? l.courses
            .map(
              (c) =>
                `<span class="status-badge" style="background: rgba(255,255,255,0.1); margin-bottom: 0.2rem; display: inline-block;">${c.course_id}</span>`,
            )
            .join(" ")
...
async function openAssignModal(courseId, courseName, department) {
  ...
  document.getElementById("assignCourseName").textContent =
    `Assigning lecturer for: ${courseName} (${courseId})`;
...
document.getElementById("assignForm")?.addEventListener("submit", async (e) => {
  ...
  try {
    const response = await adminFetch(`/api/admin/courses/${courseId}/assign`, {
      method: "POST",
      body: JSON.stringify({
        lecturer_id: lecturerId === "null" ? null : lecturerId,
      }),
    });
...
function displayAdminSchedules(schedules) {
  ...
  schedules.forEach((s) => {
    ...
    const endTime = new Date(s.class_end_time).toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
    });
...
function displayScheduleDetails(details, code, name) {
  ...
  document.getElementById("schedDetailTitle").textContent =
    `Attendance List: ${code}`;
    ...
    const statusClass =
      status === "VALID"
        ...
        : status === "ABSENT"
          ? "status-inactive"
          : "";
    const markedAt = d.marked_at
      ? new Date(d.marked_at).toLocaleTimeString()
      : "-";
...
document
  .getElementById("closeSchedDetailModal")
  ?.addEventListener("click", () => {
    document.getElementById("scheduleDetailModal").style.display = "none";
  });

document
  .getElementById("schedLevelFilter")
  ?.addEventListener("change", fetchAndDisplayScheduleDetails);
document
  .getElementById("schedDeptFilter")
  ?.addEventListener("change", fetchAndDisplayScheduleDetails);
...
function populateDepartmentSelects(departments) {
  ...
  const lecturerFilterSelect = document.getElementById("lecturerDeptFilter");
...
function displayAttendance(data) {
  ...
  data.forEach((log) => {
    const tr = document.createElement("tr");
    const statusClass =
      log.status === "VALID" ? "status-active" : "status-inactive";
  ...
  document.querySelectorAll(".detail-btn").forEach((btn) => {
    btn.addEventListener("click", () =>
      showAttendanceDetails(btn.getAttribute("data-id")),
    );
  });
}
...
document
  .getElementById("courseLevelFilter")
...
document
  ...
  ?.addEventListener("change", loadAdminSchedules);
document
  .getElementById("mainSchedDeptFilter")
  ?.addEventListener("change", loadAdminSchedules);
...
document
  .getElementById("attendanceLevelFilter")
  ?.addEventListener("change", loadAttendance);
document
  .getElementById("attendanceDeptFilter")
  ?.addEventListener("change", loadAttendance);
